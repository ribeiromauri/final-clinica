...
INSERT INTO DIAS VALUES ('Sábado')
...
ALTER PROCEDURE SP_ELIMINAR_MEDICO(
	@ID int
)
AS
BEGIN
...
BEGIN
	BEGIN TRY
		BEGIN TRANSACTION
			...
			WHERE ID = @ID
			UPDATE ADM_USUARIOS SET DNI_USUARIO = @DNI, CONTRASEÑA = @Contraseña
			WHERE ID_USUARIO = @ID
...
INNER JOIN HORARIOS_TRABAJO HT ON HT.ID_MEDICO = M.ID
INNER JOIN ESPECIALIDAD_X_MEDICO EXM ON EXM.ID_MEDICO = M.ID
INNER JOIN ESPECIALIDADES E ON E.ID = EXM.ID_MEDICO


INSERT INTO HORARIOS_TRABAJO VALUES (@DIA, @ID_MEDICO, H_ENTRADA, H_SALIDA, 0)

SELECT HT.H_ENTRADA AS H_ENTRADA, HT.H_SALIDA AS H_SALIDA, HT.LIBRE AS LIBRE FROM HORARIOS_TRABAJO HT INNER JOIN MEDICOS M ON M.ID = HT.ID_MEDICO WHERE M.ID = @ID

SELECT ID ,DIA FROM DIAS